...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      if (logsError) {
        console.error("Error fetching frisdrank_logs:", logsError);
        setLoading(false);
        return;
      }
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-6 mb-8 max-w-4xl mx-auto">
            ...
            <Card
              className={`${getRankStyle(
                1
              )} order-1 md:order-1 mt-4 md:mt-0 hover:scale-105 transition-all duration-200`}
            >
              <CardContent className="p-3 md:p-6 text-center">
                ...
                <Avatar className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-2 md:mb-3">
                  <AvatarImage
                    src={leaderboard[1].avatar_url ?? "/ND_default.png"}
                  />
                  <AvatarFallback>
                    {getInitials(
                      leaderboard[1].username,
                      leaderboard[1].profile_id
                    )}
                  </AvatarFallback>
                </Avatar>
                <h3 className="font-semibold text-sm md:text-lg mb-1 truncate">
                  {leaderboard[1].username ??
                    `User ${leaderboard[1].profile_id.slice(0, 8)}`}
                </h3>
            ...
            <Card
              className={`${getRankStyle(
                0
              )} order-2 md:order-2 md:scale-110 md:z-10 hover:scale-110 md:hover:scale-125 transition-all duration-200`}
            >
              <CardContent className="p-3 md:p-6 text-center">
                ...
                <Avatar className="w-14 h-14 md:w-20 md:h-20 mx-auto mb-2 md:mb-3 ring-2 md:ring-4 ring-yellow-200 dark:ring-yellow-800">
                  <AvatarImage
                    src={leaderboard[0].avatar_url ?? "/ND_default.png"}
                  />
                  <AvatarFallback>
                    {getInitials(
                      leaderboard[0].username,
                      leaderboard[0].profile_id
                    )}
                  </AvatarFallback>
                </Avatar>
                <h3 className="font-bold text-sm md:text-xl mb-1 truncate">
                  {leaderboard[0].username ??
                    `User ${leaderboard[0].profile_id.slice(0, 8)}`}
                </h3>
            ...
            <Card
              className={`${getRankStyle(
                2
              )} order-3 md:order-3 mt-8 md:mt-0 hover:scale-105 transition-all duration-200`}
            >
              <CardContent className="p-3 md:p-6 text-center">
                <div className="flex justify-center mb-3">{getRankIcon(2)}</div>
                <Avatar className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-2 md:mb-3">
                  <AvatarImage
                    src={leaderboard[2].avatar_url ?? "/ND_default.png"}
                  />
                  <AvatarFallback>
                    {getInitials(
                      leaderboard[2].username,
                      leaderboard[2].profile_id
                    )}
                  </AvatarFallback>
                </Avatar>
                <h3 className="font-semibold text-sm md:text-lg mb-1 truncate">
                  {leaderboard[2].username ??
                    `User ${leaderboard[2].profile_id.slice(0, 8)}`}
                </h3>
                <Badge variant="secondary" className="mb-2">
                  <Rocket className="w-3 h-3 mr-1" />
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <Avatar className="w-12 h-12">
                    ...
                    <AvatarFallback>
                      {getInitials(entry.username, entry.profile_id)}
                    </AvatarFallback>
                  </Avatar>

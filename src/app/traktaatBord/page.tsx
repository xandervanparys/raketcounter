...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      ...
      const { data: logs, error: logsError } = await supabase.from("traktaat_logs").select("profile_id, amount")

      if (logsError) {
        console.error("Error fetching traktaat_logs:", logsError)
        setLoading(false)
        return
      }
    ...
    <main className="min-h-screen bg-background p-8">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Grootste trakteerder</p>
        </div>
          ...
          <div className="grid grid-cols-3 gap-2 md:gap-6 mb-8 max-w-4xl mx-auto">
            ...
            <Card className={`${getRankStyle(0)} order-2 md:order-2 md:scale-110 md:z-10 hover:scale-110 md:hover:scale-125 transition-all duration-200`}>
              <CardContent className="p-3 md:p-6 text-center">
                ...
                <p className="text-xs text-muted-foreground hidden md:block">🍻 Grootste trakteerder</p>
              </CardContent>
            </Card>
